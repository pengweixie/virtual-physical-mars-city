...
//
// 几何即账本（设计册 E:\Claude\mars-grid：r1 11 本账 78 闸全绿；r2「从干线到插座」5 本账 26 闸，
//   2 红按名保留——账 14 的插头灭弧结论未过消融改为条件式，账 15 的 Boltzmann 混合气案未过登记闸不发数）：
...
//   · 南坪的样板配电柜是 r2 的头条 —— 全城负荷表逐卡重核后（账 12）主粮舱 260 kW 均值
//     此前根本不在表里，T1 生存层按各单位自己的依据只有 8.7 kW 加五项没人定价的生保缺口（账 16）
//
...
export function build(THREE) {
  ...
  // =================================================================
  // 9b. 舱内两级配电柜样板（r2 账 13/14/15）—— 站坪南侧展示架
  //     全城的插座级在这里定：橙 = 400 V 固定接线（固态断路器 + IMD），
  //     蓝 = 48 V 插座级（400→48 V 变流器 + 20 路回路，≤1.54 kW/32 A 插头），
  //     绿黄 = 压力壳等电位联结排 —— IT 制延伸进舱，壳是参考不是回路。
  //     完整部件函数见 viewer/units/grid-parts.js（供 hab-quarter-01 / hab-home-01 复用）；
  //     本站内联一份简版，不 import（保持契约自检 0 FAIL）。
  // =================================================================
  {
    const BX = -13.5, BZ = 13.2, BY = 0.4;                  // 展示架底面（南坪，说明牌西侧）
    const ORANGE = M(0xd8742a, { roughness: 0.6 }); const BLUE = M(0x2a6aa8, { roughness: 0.6 });
    const ledG = emissive(0x4fe86a, 1.8), ledA = emissive(0xffc878, 1.8), scr = emissive(0x4fd8e8, 1.4);
    [ORANGE, BLUE].forEach(reg);
    for (const sx of [-0.45, 0.45]) box(0.08, 0.6, 0.08, trim, BX + sx, BY + 0.3, BZ);   // 架腿
    box(1.1, 0.06, 0.4, trim, BX, BY + 0.62, BZ);                                          // 架面
    const y0 = BY + 0.65;                                                                  // 柜底
    box(0.80, 1.20, 0.02, steel, BX, y0 + 0.60, BZ - 0.115);                               // 背板
    for (const sx of [-0.39, 0.39]) box(0.02, 1.20, 0.25, steel, BX + sx, y0 + 0.60, BZ);
    box(0.80, 0.02, 0.25, steel, BX, y0 + 1.19, BZ); box(0.80, 0.02, 0.25, steel, BX, y0 + 0.01, BZ);
    box(0.36, 1.16, 0.015, steel, BX - 0.56, y0 + 0.60, BZ + 0.115, -1.9);                // 双门开 ~110°
    box(0.36, 1.16, 0.015, steel, BX + 0.56, y0 + 0.60, BZ + 0.115, 1.9);
    box(0.74, 0.06, 0.01, white, BX, y0 + 1.13, BZ + 0.105);                               // 铭牌
    box(0.30, 0.012, 0.004, ORANGE, BX - 0.18, y0 + 1.13, BZ + 0.112);
    box(0.30, 0.012, 0.004, BLUE, BX + 0.18, y0 + 1.13, BZ + 0.112);
    box(0.30, 0.008, 0.02, copper, BX - 0.22, y0 + 1.06, BZ - 0.06);                      // 400 V 母排 ±
    box(0.30, 0.008, 0.02, copper, BX - 0.22, y0 + 1.03, BZ - 0.06);
    box(0.072, 0.126, 0.084, dark, BX - 0.31, y0 + 0.88, BZ - 0.02);                       // 进线 SSCB
    box(0.064, 0.012, 0.004, ORANGE, BX - 0.31, y0 + 0.93, BZ + 0.023);
    box(0.012, 0.012, 0.004, ledG, BX - 0.30, y0 + 0.88, BZ + 0.023);
    box(0.11, 0.12, 0.07, dark, BX - 0.13, y0 + 0.85, BZ - 0.02);                          // IMD + RCM
    box(0.08, 0.04, 0.004, scr, BX - 0.13, y0 + 0.875, BZ + 0.017);                        // 绝缘电阻屏
    box(0.010, 0.010, 0.004, ledG, BX - 0.16, y0 + 0.82, BZ + 0.017);
    box(0.010, 0.010, 0.004, ledA, BX - 0.10, y0 + 0.82, BZ + 0.017);                      // 第一故障：报警不跳
    for (let i = 0; i < 6; i++) {                                                          // 400 V 馈线 SSCB ×6
      const fx = BX - 0.35 + (i % 3) * 0.06 + 0.02, fy = y0 + 0.70 - Math.floor(i / 3) * 0.11;
      box(0.045, 0.09, 0.07, dark, fx, fy, BZ - 0.02);
      box(0.040, 0.012, 0.004, ORANGE, fx, fy + 0.033, BZ + 0.017);
      box(0.010, 0.010, 0.004, ledG, fx + 0.012, fy + 0.01, BZ + 0.017);
    }
    box(0.26, 0.14, 0.12, dark, BX + 0.22, y0 + 0.93, BZ - 0.04);                          // 400→48 V 变流器
    for (let i = 0; i < 9; i++) box(0.24, 0.10, 0.004, steel, BX + 0.22, y0 + 0.93, BZ - 0.098 + i * 0.012);
    box(0.20, 0.010, 0.004, BLUE, BX + 0.22, y0 + 0.985, BZ + 0.021);
    for (let i = 0; i < 20; i++) {                                                         // 48 V 插座回路 SSCB ×20（4×5）
      const fx = BX + 0.22 - 0.13 + (i % 5) * 0.052 + 0.026, fy = y0 + 0.74 - Math.floor(i / 5) * 0.085;
      box(0.036, 0.072, 0.063, dark, fx, fy, BZ - 0.02);
      box(0.032, 0.010, 0.004, BLUE, fx, fy + 0.026, BZ + 0.012);
      box(0.008, 0.008, 0.004, i === 0 ? ledA : ledG, fx + 0.010, fy + 0.008, BZ + 0.012);   // 0 号回路握手中
    }
    for (let i = 0; i < 6; i++)                                                            // 等电位联结排（绿黄）
      box(0.62 / 6, 0.03, 0.006, i % 2 ? BONDY : BOND, BX - 0.31 + (i + 0.5) * 0.62 / 6, y0 + 0.075, BZ + 0.075);
    box(0.03, 0.03, 0.12, BOND, BX + 0.30, y0 + 0.045, BZ - 0.04);                          // 到压力壳的联结带
    box(1.0, 0.30, 0.04, white, BX, BY + 0.30, BZ + 0.22);                                  // 架前说明牌：「从干线到插座」
    box(0.9, 0.05, 0.01, BLUE, BX, BY + 0.20, BZ + 0.245);
  }
  ...
  anchor('cabin', -13.5, 1.6, 13.2);                 // 从干线到插座：舱内两级配电（r2 账 12–16）

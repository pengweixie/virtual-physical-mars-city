...
export function build(THREE) {
  ...
  /* —— 3d. 芯片厂门组（右墙实体段 z=-2.5，通 ops-fab-01）—— */
  box(0.16, 2.7, 2.6, M.doorTrim, 5.92, 1.85, -2.5);      // 门套（凸出墙内面）
  box(0.06, 2.3, 1.06, M.steel, 5.86, 1.65, -1.96);     // 双扇不锈钢门（缝 0.04）
  box(0.06, 2.3, 1.06, M.steel, 5.86, 1.65, -3.04);
  box(0.05, 0.22, 0.6, G.lift, 5.85, 3.02, -2.5);         // 门楣状态屏
  box(0.06, 0.35, 1.8, G.sign, 5.9, 3.5, -2.5);           // 「芯片厂」发光标志板
  box(0.05, 0.3, 0.14, M.doorTrim, 5.88, 1.35, -1.05);  // 门禁面板
  box(0.04, 0.06, 0.06, G.ledG, 5.85, 1.42, -1.05);
  for (let i = 0; i < 6; i++) {                         // 门前黄黑警示垫
    box(0.9, 0.02, 0.42, i % 2 === 0 ? M.hazardY : M.hazardK, 5.5, 0.515, -3.55 + i * 0.42);
  }
  /* —— 3e. 居住区门组（左墙实体段 z=-3.5，通 hab-quarter-01）—— */
  box(0.16, 2.7, 2.6, M.doorTrim, -5.92, 1.85, -3.5);
  box(0.06, 2.3, 1.06, M.steel, -5.86, 1.65, -2.96);
  box(0.06, 2.3, 1.06, M.steel, -5.86, 1.65, -4.04);
  box(0.05, 0.22, 0.6, G.lift, -5.85, 3.02, -3.5);
  box(0.06, 0.35, 1.8, G.sign, -5.9, 3.5, -3.5);        // 「居住区」发光标志板
  box(0.05, 0.3, 0.14, M.doorTrim, -5.88, 1.35, -2.05);
  box(0.04, 0.06, 0.06, G.ledG, -5.85, 1.42, -2.05);
  for (let i = 0; i < 6; i++) {
    box(0.9, 0.02, 0.42, i % 2 === 0 ? M.hazardY : M.hazardK, -5.5, 0.515, -4.55 + i * 0.42);
  }
  /* —— 3c. 量子计算中心门组（左墙实体段 z=-8，通 sci-quantum-01）—— */
  box(0.16, 2.7, 2.6, M.doorTrim, -5.92, 1.85, -8);     // 门套（凸出墙内面）
  box(0.06, 2.3, 1.06, M.steel, -5.86, 1.65, -7.46);    // 双扇不锈钢门（缝 0.04）
  box(0.06, 2.3, 1.06, M.steel, -5.86, 1.65, -8.54);
  box(0.05, 0.22, 0.6, G.lift, -5.85, 3.02, -8);        // 门楣状态屏
  box(0.06, 0.35, 1.8, G.sign, -5.9, 3.5, -8);          // 「玄枢」发光标志板
  box(0.05, 0.3, 0.14, M.doorTrim, -5.88, 1.35, -6.55); // 门禁面板
  box(0.04, 0.06, 0.06, G.ledG, -5.85, 1.42, -6.55);
  for (let i = 0; i < 6; i++) {                         // 门前黄黑警示垫
    box(0.9, 0.02, 0.42, i % 2 === 0 ? M.hazardY : M.hazardK, -5.5, 0.515, -9.05 + i * 0.42);
  }
  {

...
- **管线接驳的分工（2026-08-15 定）**：城市走廊（道路/管廊/电缆沟）是**总控的织体**，
  资产是**各 session 的**，接口就在两者交界处。规矩是**双方各修一半，在门口相遇**：
  - **总控侧**：每条管廊的真端头自带**下行立管 + 落地接入撬 + 基础垫**（引擎 `pipeRack`
    的 `ends` 参数控制；多段拼成的一条走廊只在真端头出接入撬，中间接缝传 `'none'`）。
    走廊只负责**送到门口**，不进建筑。
  - **session 侧**：如果你的资产需要"看得见的接管"，就在**自己的模块里**朝走廊方向做一段
    短接管/法兰/穿墙套管（几米即可），并在 HANDOFF 里写明期望的接驳方位与标高。
    不要自己画长距离管道——那是走廊，会与审计和别人的资产打架。
  - 谁都别做的事：**把管子插进对方的几何里**。两边都停在门口，视觉上自然接上。
...
- **双语（页面有 EN/中文 切换）**：每个字段都支持可选的 `*_en` 孪生——
  `label_en` / `detail_en` / `specs_en`（整对象）/ `sim_en` / `physics_en`，
  模块 meta 与 manifest 支持 `name_en`。英文模式下引擎自动选用，缺省回退中文。
  **新交付的卡请直接双语**；存量卡由各 session 补 `*_en` 字段即可上线
...
## 6a. 动态图（GIF）交付 —— 给所有带动画资产的通用规则

每个有动态的资产交付一张**循环 GIF**,展示它的标志性动作(发射、挖掘、巡逻、
开缝巡天、夜光点亮……)。这是资产的"动态名片",总控会挑选进 README。

**规格(硬要求)**
- 位置与命名:`snaps/anim/<资产ID>.gif`(一资产多动作时加后缀,如
  `veh-rocket-02-launch.gif`)
- 时长 8~12 s · 宽 960 px(16:9)· **≤ 8 MB**(GitHub README 加载友好);
  超了就减秒数或降到 720 宽
- 画面里**不得有 HUD/中文标签**(工具默认隐藏,勿加 --keepui)
- 尽量首尾呼应成循环(巡逻走完一圈、动作做完复位、或取稳态动态一段)

**拍法(总控已提供工具,ffmpeg 已装)**

```
node scripts/capture_gif.mjs --url "?inspect=<资产ID>&colony=1&t=15" \
     --out snaps/anim/<资产ID>.gif --seconds 10
```

- 取景:`?inspect=` 自动环视取景;要自定机位用
  `?colony=1&x=..&z=..&y=..&yaw=..&pitch=..&fly=1`(y 是地形上方高度)
- 时刻:`&t=15` 白天 / `&t=20.5` 夜景(夜光/光束/穹顶开缝要夜拍)
- 室内:`--url "?interior=hab-foyer-01"`
- **触发一次性动作**(发射等):加 `--eval` 在录制前执行,如
  `--eval "(()=>{__mars.units.find(u=>u.id==='ops-spaceport-02').group.userData.actions['发射准备']();return 1})()"`
  (工具自动附加 debug=1,页面里可用 __mars)
- 工具按真实耗时自适应帧率,动作速度所见即所得;录完自动报体积

**交付后**:CHECKLIST 自己那行备注加"动图 ✓";超过 1 张列出文件名。

...
**交付 session**:mars-thermal(2026-08-07)· 设计册真源 `E:\Claude\mars-ir`(五本账 sim/01–05 + out/*.json,已本地 git 提交,未推远端)
...
4. **坑账**:8462 截图上传服务存在旧 session 残留实例(双进程同端口),POST 会被
   旧实例抢答、文件落 `E:\Claude\mars-radio\shots\`——用完请 kill,或按落盘目录找片。
...
- **08-09 fable review 轮**:①修真 bug——云台俯仰符号反了(el.rotation.x=-0.16 实为仰视 9°,
  three.js 正 x 旋转才是前向压低;翻正 +0.16 后热屏满幅地形,size_m 实测 7.36→7.39 同步,
  validate/包络/城内 scale=1 全部重验,GIF 重拍);②「1 K 全温域 >5σ」超标声明改诚实口径
  (180 K 处 2.9σ 单帧);③09 账「差值恰等=自洽」的同义反复削弱(膜/镜拆分未独立验证);
  ④卡 τ spec 补 FEM 双口径;⑤核心数字链逐项复算全部对上
